...
def run_demo():
    ...
    t_jan1 = datetime(2026, 1, 1, 10, 0, 0, tzinfo=timezone.utc)
    ...
    try:
        with Recall(db_path=db_path, clock=clock, extractor=FixtureExtractor()) as client:
            ...
            for item in q_m1:
                fact = item.fact
                sal = item.salience
                print(f"  • {fact.predicate}: {fact.object_value} | Salience Score: {sal.composite_score} (retention={sal.retention}, similarity={sal.similarity})")

            # -------------------------------------------------------------------------
            # MONTH 3: March 1, 2026 — Independent Reinforcement & Language Addition
            # -------------------------------------------------------------------------
            t_mar1 = datetime(2026, 3, 1, 14, 30, 0, tzinfo=timezone.utc)
            clock.set(t_mar1)
            ...
            # -------------------------------------------------------------------------
            t_may1 = datetime(2026, 5, 1, 9, 0, 0, tzinfo=timezone.utc)
            clock.set(t_may1)
            ...
            # -------------------------------------------------------------------------
            t_jun1 = datetime(2026, 6, 1, 18, 0, 0, tzinfo=timezone.utc)
            clock.set(t_jun1)
            ...
            t_may31 = datetime(2026, 5, 31, 23, 59, 59, tzinfo=timezone.utc)
            ...
            for item in q_before_corr:
                print(f"   - {item.fact.predicate}: {item.fact.object_value} (version_id={item.fact.version_id}, source={item.fact.source_ref})")
            ...
            for ver in history:
                ...
                print(f"   [Version {ver.version_id}] val={ver.object_value} | Valid: [{ver.valid_from.strftime('%Y-%m-%d')} to {v_to_str}) | Tx: [{ver.tx_from.strftime('%H:%M:%S')} to {t_to_str}) | Source: {ver.source_ref}")
            ...
            q_m6_all = client.query(about_entity="John", context="Where does John work and what languages does he speak?", valid_at=t_jun1, known_at=t_jun1)
            ...
            for item in q_m6_all:
                ...
                print(f"    Composite Salience: {s.composite_score} (Similarity: {s.similarity}, Retention R(t): {s.retention}, Confidence: {s.confidence})")
            ...
            if employer_fact and languages:
                ...
                print(f"       referencing his role transition.\"")
