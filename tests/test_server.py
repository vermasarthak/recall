...
def test_successful_ingest_and_query(client):
    ...
    # 1. Ingest
    ingest_payload = {"speaker": "John", "text": "John works at Google", "conversation_id": "c1"}
    ...
    # 2. Query
    query_payload = {"about_entity": "John", "context": "Where does John work?"}
...
def test_multi_tenant_isolation(client):
    ...
    client.post(
        "/api/v1/ingest",
        headers=headers_a,
        json={"speaker": "Alice", "text": "Alice likes apples", "conversation_id": "c1"},
    )
...
def test_search_endpoint(client):
    ...
    client.post(
        "/api/v1/ingest",
        headers=headers,
        json={"speaker": "John", "text": "John likes StarWars", "conversation_id": "c1"},
    )

    # Global Search
    search_payload = {"query": "watching movies", "min_salience": 0.0, "limit": 5}
...
def test_tenant_id_with_underscore_and_hyphen(client):
    ...
    ingest_payload = {"speaker": "John", "text": "John lives in London", "conversation_id": "c1"}
    response = client.post("/api/v1/ingest", headers=headers, json=ingest_payload)
    assert response.status_code == 200
