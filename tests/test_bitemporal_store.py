...
import pytest
from recall.config import TestClock, NaiveDatetimeError
...
def test_bitemporal_half_open_intervals(tmp_path):
    clock = TestClock(datetime(2026, 1, 1, 0, 0, 0, tzinfo=timezone.utc))
    store = StorageEngine(str(tmp_path / "test.db"), clock=clock)
    ...
    v_from = datetime(2026, 1, 1, 0, 0, 0, tzinfo=timezone.utc)
    v_to = datetime(2026, 7, 1, 0, 0, 0, tzinfo=timezone.utc)
    tx_time = datetime(2026, 1, 1, 0, 0, 0, tzinfo=timezone.utc)
    ...
    march_1 = datetime(2026, 3, 1, 0, 0, 0, tzinfo=timezone.utc)
...
def test_knowledge_time_versioning_and_historical_correction(tmp_path):
    ...
    march = datetime(2026, 3, 1, 0, 0, 0, tzinfo=timezone.utc)
